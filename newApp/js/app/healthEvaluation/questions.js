		...
		methods: {
			...
			manageQuestion: function(data) { //处理问题
				...
				if(data) {
					...
					for(var i = 0, n = data.length; i < n; i++) {
						var now = data[i];
						var id = now.id;
						var parentid = now.parentid;
						if(data[i].parentid == 0) {
							that.question_o.push(now);
						} else {
							ob_t[parentid] ? "" : ob_t[parentid] = [];
							ob_t[parentid].push(now);

						}

					}
					that.question_t = ob_t;
					console.log(that.question_t);
				}
			},
			...
			chooseActive: function(is_question, is_option) { //处理选中状态
				...
				for(var i = 0, n = this.option_p[id].length; i < n; i++) {
					switch(se_type) {
						case 0:
							this.option_p[id][i].isSelected = false;
							is_option.isSelected = true;
							break;
						case 1:
							is_option.isSelected ? is_option.isSelected = false : is_option.isSelected = true;
							break;
						default:
							;
